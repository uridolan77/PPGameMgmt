    ...
    static void Main(string[] args)
    {
        ...
        try
        {
            ...
            using (var connection = new MySqlConnection(serverConnectionString))
            {
                ...
                var dropDbCommand = new MySqlCommand("DROP DATABASE IF EXISTS pp_recommeder_db;", connection);
                dropDbCommand.ExecuteNonQuery();
                Console.WriteLine("Dropped database if it existed");

                // Create database
                var createDbCommand = new MySqlCommand("CREATE DATABASE pp_recommeder_db;", connection);
                createDbCommand.ExecuteNonQuery();
                Console.WriteLine("Database created");
            }

            // Use the new database and create tables
            string dbConnectionString = "Server=localhost;Database=pp_recommeder_db;User=root;Password=Dt%g_9W3z0*!I";
